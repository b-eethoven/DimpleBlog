...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        com.asiainfo.config.datasource.DynamicDataSourceContextHolder.dataSourceIds.add("master");
        logger.info("注册默认数据源成功");
        // 获取其他数据源配置
        List<Map> configs = binder.bind("spring.datasource.qm", Bindable.listOf(Map.class)).get();
        registerDatasource(configs);
        List<Map> oracleConfigs = binder.bind("spring.datasource.oracle", Bindable.listOf(Map.class)).get();
        registerDatasource(oracleConfigs);

        // bean定义类
        GenericBeanDefinition define = new GenericBeanDefinition();
        // 设置bean的类型，此处DynamicRoutingDataSource是继承AbstractRoutingDataSource的实现类
        define.setBeanClass(com.asiainfo.config.datasource.DynamicDataSource.class);
        // 需要注入的参数
    ...
    private void registerDatasource(List<Map> configs) {
        ...
        DataSource consumerDatasource;// 遍历从数据源
        for (int i = 0; i < configs.size(); i++) {
            ...
            com.asiainfo.config.datasource.DynamicDataSourceContextHolder.dataSourceIds.add(key);
